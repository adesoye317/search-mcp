...
export function getOptions() {
  ...
  // Force HTTP transport in cloud environments
  const transport = isCloudEnvironment 
    ? 'http' 
    : (process.env.TRANSPORT || process.env.MCP_TRANSPORT || 'stdio');

  // CRITICAL: Use PORT from environment first, then fall back to default
  ...
  const options = {
    ...
    disabledTools: process.env.DISABLED_TOOLS?.split(',') || [],
    port,  // This should now be 3000 on Render
    host: process.env.HOST || '0.0.0.0'
  };

  console.log('Configuration loaded:', {
    ...options,
    braveApiKey: '***' // Don't log the actual key
  });
